  ...
  for (subject in 1:Nsubjects){
    ...
    if (first_trial_in_block[subject,trial] == 1) {
      ...
      Q_keys=Q_keys_initial;
          ...
          Qnet[1]=(1 + slope_rel[subject] * capacity[subject])*Q_cards[card_left[subject,trial]]+(intercept_irrel[subject] +
          slope_irrel[subject]*capacity[subject])*Q_keys[1] ;
          Qnet[2]=(1 + slope_rel[subject] * capacity[subject])*Q_cards[card_right[subject,trial]]+(intercept_irrel[subject] +
          slope_irrel[subject]*capacity[subject])*Q_keys[2];
